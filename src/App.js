...
class App extends React.Component {
  ...
  prepareCards() {
    ...
    return [...config.cards, ...config.cards]
      .sort(() =>  Math.random() - 0.5 ) // один из способов сортировки массива в рандомной порядке
    ...
    return (
      <div className="App">
          ...
          <div className="cards">
            {
              this.state.cards.map(item => (
                <Card item={item} key={item.id} isShown={item.isOpened || item.isCompleted}
        ...
        <Popup open={this.state.isPopupOpened} closeOnDocumentClick onClose={this.closePopup.bind(this)}>
          <div className="modal">
            <span className="close" onClose={this.closePopup.bind(this)}>
              &times;
            </span>
